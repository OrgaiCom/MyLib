    ...
    public class LibCsv
    {
        ...
        public void Write(DataTable dt, string csvFilePath, bool writeHeader, string delimiter = ",", string encodeName = "shift-jis", bool isAppend = false)
        {
            ...
            using (StreamWriter sw = new StreamWriter(csvFilePath, isAppend, Encoding.GetEncoding(encodeName)))
            {
                ...
                if (header)
                {
                    string[] headers = dt.Columns.Cast<DataColumn>().Select(i => EncloseIfNeed(i.ColumnName)).ToArray();
                ...
                {
                    string[] fields = Enumerable.Range(0, dt.Columns.Count).Select(i => EncloseIfNeed(dr[i].ToString())).ToArray();
        ...
        private string EncloseIfNeed(string strField)
        {
            //ダブルクォートで括る必要があるかを確認
            if (strField.Contains('"') || strField.Contains(',') || strField.Contains('\r') || strField.Contains('\n') ||
                 strField.StartsWith(" ") || strField.StartsWith("\t") || strField.EndsWith(" ") || strField.EndsWith("\t"))
            {
                //ダブルクォートが含まれていたら２つ重ねて、前後にダブルクォートを付加
                return (strField.Contains('"')) ? ("\"" + strField.Replace("\"", "\"\"") + "\"") : ("\"" + strField + "\"");
            }
            else
            {
                //何もせずそのまま返す
